...
## Sprint 223 (v2.211.0) — Settings modal: dialog semantics + focus management
- [ ] Accessibility: the Settings modal had NO focus management — openSettings just toggled .show. No role=dialog, no focus moved into it, no Escape-to-close, no focus return. A keyboard/SR user opening Settings stayed focused behind the modal and could Tab into the page underneath; opening was barely announced
- [ ] Settings .modal now has role="dialog" aria-modal="true" aria-labelledby="settingsTitle" (h3 got id). openSettings stores document.activeElement, shows, and focuses the first control inside (preventScroll). closeSettings returns focus to the opener. Escape closes it (keydown handler, after the Brew-Along Escape branch). Matches the app's existing focus-into+Escape pattern (no JS Tab-trap exists app-wide — aria-modal marks outside inert; a global Tab-trap is a separate sweep)
- [ ] npm run check green (2.211.0 synced); verified via preview: open → activeElement is inside #settingsModal + modal carries role/aria-modal/aria-labelledby resolving to "Settings"; close → focus returns to the opener button; a dispatched Escape keydown closes the modal when shown; openSettingsToKey still lands on the key field
- [ ] Update toast after deploy (cache v2.211.0)

## Sprint 224 (v2.212.0) — accessible-modal helper + saveChoice/scan modals
- [ ] Accessibility (continued from v2.211): factored a reusable focusModal(id)/blurModal(id) pair (one _modalOpener; focus into the dialog on open, return to opener on close) and applied it to the two highest-value modals lacking a11y — saveChoiceModal (the consent-on-save modal, core trust, hit every param-save) and scanModal (camera OCR)
- [ ] Both .modals now carry role="dialog" aria-modal="true" aria-labelledby (saveChoiceTitle added; scanTitle already existed). openSaveChoice/openScan call focusModal; closeSaveChoice/closeScan call blurModal (saveChoice() routes through closeSaveChoice; openScan keeps scanReset+keyhint setup). Escape closes each (keydown branches after the settings one). Settings (v2.211) untouched; compareModal + importModal queued next
- [ ] npm run check green (2.212.0 synced); verified via preview: focusModal('saveChoiceModal'/'scanModal') shows + moves focus inside; both carry role/aria-modal/aria-labelledby resolving to their titles; blurModal returns focus to the opener; dispatched Escape closes each when shown
- [ ] Update toast after deploy (cache v2.212.0)

## Sprint 225 (v2.213.0) — modal a11y sweep complete: compareModal + importModal
- [ ] Accessibility (finishes v2.211/2.212): compareModal + importModal now use focusModal/blurModal + role="dialog" aria-modal="true" aria-labelledby (compareTitle, importTitle added). openCompare→focusModal, closeCompare→blurModal (keeps compareA=null+renderLibrary); checkHashImport→focusModal, accept+declineHashImport→blurModal (replace_all). Escape: compare→closeCompare, import→declineHashImport (each its OWN close fn). importModal especially matters — a SR user (new visitor via a #r= share link) now hears the "Recipe received" dialog open
- [ ] Note: closeCompare's focus-return degrades to body (renderLibrary re-renders the opener card) — blurModal no-ops gracefully on the detached element; the open focus-into + Escape are the wins. All 6 modal-overlay dialogs (settings/saveChoice/scan/compare/import + onboarding/brew-along overlays) now have focus-into + Escape + aria-modal
- [ ] npm run check green (2.213.0 synced); verified via preview: focusModal('compareModal'/'importModal') shows + focus inside; both role/aria-modal/aria-labelledby resolve to titles; Escape closes each (compare via closeCompare→renderLibrary, import via declineHashImport→pendingImport cleared)
- [ ] Update toast after deploy (cache v2.213.0)

## Sprint 226 (v2.214.0) — gear profiles capture a custom grinder's range
- [ ] Advanced Coffee User: gear profiles (saved grinder+brewer setups) saved grinderName/grindSize/grindCoarseness/filterType/method but NOT a custom grinder's grindCustomMin/Max/Unit. A custom grinder has no GRINDER_DATA range, so applying a saved setup lost the scale → grindRange() null → grind bar + finer/coarser steppers go blank. A serious multi-device user with a hand grinder ("clicks 0-40") couldn't round-trip their kit
...
## Sprint 230 (v2.218.0) — version-diff card coaches one-variable discipline
- [ ] Emotional Design: the "changes from previous version" card (renderVersionDiff, shown after every Brew Again with changes) listed the changed fields + the rating outcome + a revert-when-worse button + the dial-in journey arc — rich, but it never reinforced the app's core one-variable-at-a-time philosophy when SEVERAL variables changed at once. The compare modal got this lesson in v2.208; the higher-frequency iteration card hadn't
- [ ] renderVersionDiff now adds a quiet .diff-discipline note based on the count of changed DIFF_FIELDS: 1 → "One change this round — a clean test you can trust" (affirms the disciplined experiment); >=2 → "N variables changed — to learn what each does, change just one next time" (gentle coach). Conditional (0 changes keeps the existing "adjust one variable" empty state); subtle italic secondary text, placed after the change lines
- [ ] npm run check green (2.218.0 synced); verified via preview (synthetic parent+child recipes): 1 changed field → "a clean test you can trust"; 2 changed → "2 variables changed … change just one next time"; 0 changed → no discipline note (empty state path)
- [ ] Update toast after deploy (cache v2.218.0)

## Sprint 231 (v2.219.0) — import rejects foreign files (don't add a blank garbage recipe)
- [ ] Product Strategist / data integrity (restore = the moat): importRecipesFile's catch already failed safe on malformed JSON ("Invalid file format", data untouched). But normalizeRecipe only rejected non-objects/arrays, then built a recipe with empty defaults for ANY object — so a FOREIGN JSON (another app's export, a settings file) hit the [data] single-object path and imported as a blank garbage recipe, reported as "Restored 1 recipe"
- [ ] normalizeRecipe now requires a non-empty string method (every BrewCraft recipe has one; shared/seeded/template recipes all do) → foreign objects return null, no garbage added. Also import now distinguishes the "nothing imported" cases: validRecipes>0 or a recognizable bundle (type brewcraft-backup / recipes|beans|gear|tags arrays) → "Already up to date"; otherwise → "That file isn't a BrewCraft backup — nothing was imported. Your recipes are untouched."
- [ ] npm run check green (2.219.0 synced); verified via preview: normalizeRecipe({foo:'bar'}) and ({name:'x'} without method) → null; normalizeRecipe(valid recipe with method) → object; the _recognized boolean is true for a backup bundle / recipe array / single valid recipe and false for a foreign object
- [ ] Update toast after deploy (cache v2.219.0)

## Sprint 232 (v2.220.0) — bottom-nav active tab announced to screen readers (aria-current)
- [ ] Mobile Kitchen / Accessibility: the bottom tab bar is ergonomically solid (fixed bottom + safe-area inset, ~48px flex:1 thumb targets, clear active state via accent + indicator bar + icon lift, Brew live-dot, switchTab doesn't stop the timer). But switchTab set aria-selected on plain <button>s — not a valid ARIA state for the button role — so a screen reader wasn't reliably told which view is active
...
## Sprint 250 (v2.238.0) - eyes-free brew: the final pour step speaks the steep/draw-down hand-off
- [ ] Accessibility / Mobile Kitchen: during-brew SR coverage was already strong - brewGuideSr (sr-only, role=status, aria-live=polite) announces each pour step debounced on change, a final-5s "X coming up" heads-up, and an espresso/pour-over-specific completion; the timer display is aria-live=off so 0.1s ticks don't flood. Gap: on the FINAL pour step the VISUAL shows both "pour to X g" AND the steep/draw-down expectation ("steeping - done ~4:00" / "drawing down - done ~4:00" / "let it finish"), but the SR announced only "[label], pour to X g" - so an eyes-free brewer was never told they'd reached the last pour or roughly when the brew finishes (worst on immersion, where a long passive steep follows the last pour)
- [ ] Fix (updateBrewGuide, ~2147): when the current step has no next (!next), append the same hand-off the visual shows to the SR announce - immersion -> ". Then let it steep[, done about M:SS]", pour-over -> ". Then let it draw down[, done about M:SS]"; the ", done about M:SS" suffix only when brewTargetSec > 0. Reuses isImmersion + secToStr + brewTargetSec (same inputs as the visual branch). Still debounced via lastGuideAnnounce (announced once on entering the final step, not per tick). Espresso unchanged (its own early-return branch with the "cut it" tone). Non-final steps unchanged
- [ ] Tradeoff: the SR text is slightly longer on the final step, but it is a one-time polite announce (not repeated), and it closes a real SR/visual parity gap the code already aims for (its own "SR parity" comment). Chose SR-natural wording ("let it steep" vs the visual "steeping")
- [ ] npm run check green (2.238.0 synced); verified via preview: final step (V60) -> "Final pour, pour to 350 g. Then let it draw down, done about 4:00"; brewTargetSec=0 -> omits "done about"; a non-final (Bloom) step -> "Bloom, pour to 50 g" with no hand-off suffix
- [ ] Update toast after deploy (cache v2.238.0)
## Sprint 251 (v2.239.0) - cupping form: no false grade on a half-filled card
- [ ] Advanced / honest-signal: the SCA cupping form (7 quality attributes 6-10; three cup columns 0-5 cups x2 pts; taints -2 / faults -4; total /100) computes correctly for a COMPLETE card (verified 7x8 + 30 = 86.00 Excellent). But calcCupping named the SCA grade band (Outstanding/Excellent/Very good/Below specialty) on EVERY keystroke during entry - and blank quality attributes count as 0 (an unscored attribute is not a 0; the SCA floor is 6). So while filling the card one attribute at a time, the cupper saw "Below specialty" on a coffee they were scoring as excellent, and a 6-of-7 card (forgot one) read "78.00 Below specialty" - a verdict it had not earned. Happens on every cupping session (each passes through the partial state), not just rare partial saves
- [ ] Fix (calcCupping): count filled quality attributes; while < 7, show the running total with "N of 7 scored" and NO grade band; name the SCA band only once all 7 are in. Same honest-signal intent as the existing "nothing filled -> dash" guard, extended to the partial state. Display-only - the math (cuppingScore) and the complete-card behavior are unchanged
- [ ] Tradeoff: kept the running total visible during entry (labeled "N of 7 scored") rather than hiding it - cuppers like watching it build and the label makes clear it is partial. Left cuppingScore (journal/card SCA for a SAVED cupping) alone - a saved partial cupping is rare, shows a bare number not a grade, and changing it would touch multiple surfaces for an edge case
- [ ] npm run check green (2.239.0 synced); verified via preview: 0 filled -> dash; 3/7 -> "54.00 3 of 7 scored" (no band); 6/7 -> "78.00 6 of 7 scored" (no band); 7/7 -> "86.00 Excellent". The · separator renders as a middot
- [ ] Update toast after deploy (cache v2.239.0)
## Sprint 252 (v2.240.0) - milestone celebrations are restore-safe (no stale "50 cups" after import)
- [ ] Emotional / honest-signal: the celebration set is otherwise well-built - 5-star and new-lineage-best fire celebrate() with specific earned messages (the climb arc "v3 is your best yet, up from 2-star"), firstrated and milestones fire ONCE, and _ratingToast defers a milestone if a best/first toast already claimed the moment (toasts share one element). firstrated is explicitly restore-guarded (only fires when no rated recipes exist). But the cumulative milestone (10/25/50/100/250 rated cups) had NO equivalent guard: a user restoring a 50-recipe backup onto a new phone has brewcraft_milestone=0 there, so their next rating computes total ~51 and fires "50 cups. You have brewed your way to instinct." - a threshold crossed by IMPORTING, not by brewing here. Asymmetric with the firstrated guard the author clearly intended
- [ ] Fix (importRecipesFile, after merge + saveToStorage, gated on added>0): record the highest already-crossed threshold - _rated = current rated count, _crossed = [10,25,50,100,250].filter(m <= _rated).pop(); if _crossed > stored milestone, set brewcraft_milestone = _crossed. So a restored backup's already-earned milestones do not re-fire, while the NEXT genuine milestone (e.g. 100) still celebrates when truly reached. setRating's milestone flow is untouched
- [ ] Tradeoff: import-time pre-set rather than a runtime brewed-vs-imported check in setRating (setRating cannot tell the difference; the import is the right place to record state). Below the first threshold (restore 8) or an all-unrated import sets nothing, so a user who genuinely brews to 10 still gets the 10-cup moment. Left firstrated alone (already runtime-guarded)
- [ ] npm run check green (2.240.0 synced); verified via preview (replicated logic): crossed(8)=none, crossed(51)=50, crossed(110)=100, crossed(300)=250; after import sets milestone=50, a rating at total 51 does NOT fire (50 not > 50), but total 100 DOES fire (100 > 50)
- [ ] Update toast after deploy (cache v2.240.0)
## Sprint 253 (v2.241.0) - bag tracking counts espresso shots (timed, not just rated)
- [ ] Mobile Kitchen / Advanced / espresso-specificity: bag tracking is otherwise solid - maybeDeductBag deducts the dose ONCE per brewed recipe (bagDeducted flag, reset on brewAgain so a new version re-deducts), wired into both the autosave-in-place path and saveRecipe; left = max(0, size - used) (no negatives); a "running low, reorder soon" warning at <=2 brews; a warm one-time end-of-bag moment on the crossing. But the "brewed" trigger was (rating > 0) || totalBrewTime - and totalBrewTime is the POUR-OVER field. The timer logs an espresso shot to espShotTime (line ~3293, "the field the dial-in reads, not the generic brew-time"), leaving totalBrewTime empty. So an espresso shot pulled + timed but not yet rated did NOT deduct; during dial-in (pull many, rate few) the bag stayed over-stated and "running low" fired late
- [ ] Fix (maybeDeductBag): brewed now also true when espShotTime is set - (rating > 0) || totalBrewTime || espShotTime. A timed shot counts even before it is rated. The bagDeducted flag still guarantees one deduction per version (no double-count), and the deduction is still scoped to a real brew (a freshly loaded/edited recipe with neither time nor rating does not deduct)
- [ ] Tradeoff: keyed on espShotTime (a RESULT, cleared on brewAgain) rather than method==Espresso + any field, so it only counts shots actually timed/entered, not espresso recipes merely opened. A shot never timed AND never rated still does not deduct - correct (no brew happened)
- [ ] npm run check green (2.241.0 synced); verified via preview (real maybeDeductBag + synthetic bean, restored after): espresso espShotTime=28 unrated -> deducts 18g; bagDeducted=true -> no second deduct (stays 18); no rating/no times -> 0; pour-over totalBrewTime=3:00 -> deducts 18
- [ ] Update toast after deploy (cache v2.241.0)
## Sprint 254 (v2.242.0) - light-theme warning text meets WCAG AA contrast
- [ ] Accessibility: theme system is sound (effectiveTheme resolves light/dark/auto; auto follows prefers-color-scheme + a change listener; persists). But in the LIGHT theme, --warning (#9a7424) measured only 3.99:1 on the card bg (#fbf6ee) - below WCAG AA 4.5:1 for normal text - and worse on input (3.34) and surface (3.07). --warning is used for small body text: .autosave-status.unsaved, .pour-total .pour-off (on --bg-surface), .bag-status.low (on --bg-input), the gesture-hint (line 81), and the fresh-degassing badge. (It is also the rating-star color, but stars are large glyphs needing only 3:1, which they clear.)
- [ ] Fix: darkened light-theme --warning #9a7424 -> #7a5810 (one line in the html.theme-light palette). Verified contrast now passes AA on all three light bgs: card 6.04, input 5.07, surface 4.66. Stars become a deep gold (still clear their 3:1 large-glyph threshold at 6.04, still read golden). Light-theme only - dark-theme --warning untouched. The fresh-degassing badge (warning text on a light amber-tint bg) improves too
- [ ] Tradeoff: chose #7a5810 as the LIGHTEST dark-amber that clears 4.5 on the darkest light bg (surface), to keep the rating stars as golden as possible rather than over-darkening to brown. The toast.warning border (decorative, not text) just gets a slightly darker accent
- [ ] Follow-up (not shipped): light-theme --success (#3f7a3f) and --danger (#b04a3a) PASS AA on the card (4.8, 5.02) but are tighter on input (4.03, 4.21) and surface (3.7, 3.87); they appear mostly on the card or on their own tinted badge bgs (fresh-peak / fresh-fading) where contrast holds - flagged to check whether either is ever raw text on input/surface
- [ ] npm run check green (2.242.0 synced); verified via preview: theme-light --warning resolves to #7a5810, ratios card 6.04 / input 5.07 / surface 4.66 (all >= 4.5)
- [ ] Update toast after deploy (cache v2.242.0)
## Sprint 255 (v2.243.0) - light-theme success + danger text meet WCAG AA (completes the status-color pass)
- [ ] Accessibility (follow-up to Sprint 254's warning fix): light-theme --success (#3f7a3f) and --danger (#b04a3a) passed on the card but fell short on darker light bgs. Measured the REAL rendered backgrounds of each color:var(--success/--danger) text element in light theme. Key finding: the Brew-Along overlay is LIGHT (--bg-deep #f1e9dc) in light theme, not dark, so there is NO dark-bg usage that a darken would harm (the concern that nearly stopped the fix). Confirmed real fails: --success on --bg-deep = 4.29 (autosave "saved" status 11px; .ba-next 13px) and on --bg-surface = 3.70 (.pour-match). Danger's text sits mostly on the card / its own tinted badges but was darkened for consistency since no dark-bg conflict exists
- [ ] Fix: light-theme --success #3f7a3f -> #2f6a2f, --danger #b04a3a -> #a04030 (same html.theme-light palette line as the v2.242 warning fix). Chose the lightest green/red that clears 4.5 on the darkest light bg (surface), so each stays clearly green/red. Light-theme only; dark theme untouched. Darker text also improves contrast on the green/red badge tints (fresh-peak, fresh-fading) and on the light Brew-Along overlay
- [ ] Tradeoff: darkened both even though danger's confirmed text usages were card/tints (where it passed) - consistency + robustness, and the overlay being light removes any dark-bg downside. Did not touch the dark theme (separate audit if ever warranted)
- [ ] npm run check green (2.243.0 synced); verified via preview (theme-light, worst ratio across card/input/surface/deep): success #2f6a2f = 4.67, warning #7a5810 = 4.66, danger #a04030 = 4.60 - all three status colors now >= 4.5 on every light bg
- [ ] Update toast after deploy (cache v2.243.0)
## Sprint 256 (v2.244.0) - dark-theme danger text meets WCAG AA (completes contrast pass across both themes)
- [ ] Accessibility: ran the same WCAG contrast measurement on the DARK theme (the default) that fixed the light theme in v2.242-243. Most roles pass on the lightest dark bg (surface #2e261d): text-muted 4.86 (the line-849 #a1917c override), secondary 5.3, primary 12.07, success 6.35, warning 6.33. But --danger (#c46b5e) measured 3.98:1 on surface - below AA 4.5:1 for normal text - and only ~4.5 on the card, where its real text usages live (.btn-danger, .scan-status.err, .fresh-fading on its own red tint)
- [ ] Fix: lightened dark-theme --danger #c46b5e -> #cf7a6c (the :root default at line 27; the v9 :root block at 697 overrides bgs but not danger; the light theme keeps its own --danger #a04030 at 827, untouched). Light-on-dark, so lightening raises contrast on EVERY dark bg uniformly - verified worst-case now 4.71 (surface), 5.38 (card), 5.89/5.98 (deep/input). Chose the most-saturated coral that clears 4.5 so it still reads as a danger red, not a washed-out pink
- [ ] Tradeoff: lightened (not darkened) because dark theme is light-on-dark - the opposite of the light-theme danger fix which darkened. No light-bg usage exists in dark theme, so no conflict. Left success/warning/text-muted alone (they pass)
- [ ] npm run check green (2.244.0 synced); verified via preview (dark, worst across deep/card/input/surface): danger #cf7a6c = 4.71, success 6.35, warning 6.33, text-muted 4.86 - all >= 4.5. BOTH themes now pass AA across text roles + status colors
- [ ] Update toast after deploy (cache v2.244.0)
## Sprint 257 (v2.245.0) - small action buttons (.btn-sm) reach the 44px tap target
- [ ] Mobile Kitchen: systematically measured every visible interactive element's rendered size against the 44px one-handed tap-target guideline. Almost everything held (.field inputs/selects 44, .icon-btn 44, method-cat-head 44, dial-reset/preset-chip 44+, regular .btn 44 via the v5-ergonomics rule at line 472). The genuine miss: .btn-sm carried an explicit min-height:38px (line 473) overriding .btn's 44 - so Save bean, Save setup, Del, +Add, Import Recipe and other small buttons rendered ~38px (measured the +Add button at 38), below thumb size AND misaligned with the 44px selects beside them in the profile rows
- [ ] Fix: .btn-sm min-height 38px -> 44px (one value, line 473). Buttons are display:inline-flex + align-items:center, so the label stays centered as they grow; now matches the 44px form selects/inputs they sit next to. Reverted a redundant base-.btn edit once I found line 472 already sets .btn to 44 - the real override was line 473
- [ ] Left the chips (.tag flavor/issue descriptors 33px tall) and rating stars (39px wide x 64px tall) as-is: chips clear WCAG AA target-size (24px) and forcing them to 44 would balloon a 24+ chip cloud (clutter); the stars' narrow width is offset by a 64px-tall hit area and forgiving adjacent resolution. Buttons were the clean, no-clutter win
- [ ] npm run check green (2.245.0 synced, exit 0); verified via preview: across Taste/Journal/Recipe tabs every visible .btn now renders >= 44px tall (was 38 for btn-sm)
- [ ] Update toast after deploy (cache v2.245.0)
## Sprint 258 (v2.246.0) - REDESIGN Move 1: Brew tab fully embodies the active recipe (method-adaptive)
- [ ] Super-app redesign begins. Rollback anchor: git tag checkpoint-pre-redesign-v2.245.0. PLATFORM DECISION: STAY on the single-file static PWA; add an in-platform reactive state-coordination layer (~150-line subscribe/notify) over sprints, Brew tab first; do NOT migrate to a framework (adversarial debate + judge ruling: migration is high-risk/low-user-payoff and the redesign work - smart defaults, espresso summaries, surfacing fields - is identical on any platform; a framework solves a problem BrewCraft does not have). Revisit Preact+htm (no-build) only later if cross-tab component reuse becomes a real need.
- [ ] Burden erased (the #1 burden): MEMORY/INTERPRETATION/ATTENTION. Before, the Brew tab showed name + "method · dose · 1:ratio · water" + a Dose/Ratio/Water/Temp bar but NOT grind, NOT filter; the summary was pour-over-shaped for every method, so an espresso shot advertised "1:16 -> 250g water". Grind/filter required leaving Brew -> Edit -> scrolling into beginner-hidden params; espresso brewers read the wrong four numbers; temp was a bare unit-less "93 deg".
- [ ] Fix (index.html: renderToday meta block ~6186; new renderBrewSetup + fmtTempUnit + grinderUnitLabel + bb2Tap/bbSetStat helpers near renderBrewBar ~6777; #todaySetup added to #todayHero; #brewBarBrew stat spans given generic ids bb2l0-3/bb2v0-3; .brew-setup CSS). (1) Meta line method-adaptive: pour-over "V60 · 15g · 1:16 · -> 240g water · 94C"; espresso "Espresso · 18g -> 36g · 1:2 · 28s · 93C" (dose->yield, shot time, ratio; never pour-over water). (2) New read-only "Set up" line surfaces GRIND (COARSE_LABELS descriptor + tactile anchor + exact grinder setting, e.g. "Medium · like coarse sand · 22") and FILTER (recipe filter, or "Portafilter basket" for espresso). (3) Brew-tab stat strip reshapes per method (espresso Dose/Yield/Shot/Temp, taps retarget to espYield/espShotTime); every temp now carries its C/F unit. Read-only - Edit stays the only way to change params (consent model intact). bbSet given a null guard.
- [ ] Verified via preview: pour-over + espresso meta/setup/bar all correct (exact grind "22", filter, espresso reshape, taps -> espYield/espShotTime, degF labels "201F"); screenshot confirms clean layout. npm run check green (2.246.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.246.0)
## Sprint 259 (v2.247.0) - REDESIGN Move 2: read-only pour-plan card on the Brew tab (+ Brew-Along never dead-ends)
- [ ] Burden erased: SCROLL/INTERPRETATION (sev 5) - the pour plan was invisible before Start (only #sec-pours, a collapsed block of editable 4-field cards); pour SHAPE could only be reconstructed by reading every weight and doing arithmetic. Also removes the Brew-Along dead-end (it bailed to a toast when a recipe had dose+water but no pour weights opened - common, since most quick recipes carry no structured pours).
- [ ] Fix (index.html): derivePourPlan() (read-only default schedule from dose+water: bloom = max of 2x dose / 10% water, then 2 even pours; NEVER writes the form), pourPlanDone() ("done ~M:SS" from totalBrewTime or last pour + pad), renderPourPlan() (a read-only card in #todayHero: cumulative targets "Bloom 50g -> 150g -> 250g · done ~2:30" + a proportional bar so bloom-heavy/tapered/even reads at a glance; a "suggested" tag when derived). Called from renderToday + updatePourTotals (live-updates while editing pours). Espresso + long-steep hidden (their flow is the shot line / steep note). enterBrewAlong now falls back to derivePourPlan() when buildBASteps is empty + pour-over, so Brew-Along always has steps (no dead-end) without mutating the recipe.
- [ ] Verified via preview eval: derived "suggested Bloom 30g -> 135g -> 240g" (cumulative 30/135/240 from dose15/water240); explicit pours "Bloom 45g -> 150g -> 250g · done ~2:30"; espresso card hidden; Brew-Along derives 3 steps for a waterful pour-over. npm run check green (2.247.0 synced, boots clean, exit 0). Screenshot tool timed out (renderer hiccup); functional verification complete.
- [ ] Update toast after deploy (cache v2.247.0)
## Sprint 260 (v2.248.0) - REDESIGN Move 3: method -> filter defaulting (system connectedness)
- [ ] Burden erased: DECISION/TYPING/MEMORY - picking a method never defaulted the filter; the user hand-picked e.g. "the V60 filter for a V60" from a ~40-option grouped <select>. Pairs with Move 1 (which surfaces the filter on the Brew tab) - the surfaced filter is now meaningful instead of blank.
- [ ] Fix (index.html): FILTER_DEFAULTS map (method -> canonical filter option) + maybeDefaultFilter(method), called from selectMethod (the USER method-pick handler, NOT the programmatic setMethod used on load, so loads are never dirtied). Fills filterType ONLY when blank (never overwrites a chosen filter - consent), and only if the option exists. Covers V60/Chemex/Kalita/Origami/Melitta/AeroPress/Hario Switch/French Press/Siphon/Espresso + lever (Flair/Robot/Nanopresso)/Turkish/Cupping; methods with no single obvious paper (Moka, Cold Brew/Drip, Clever, Phin) stay blank for the user.
- [ ] Verified via preview: V60->"Hario V60 02 White (Tabbed)", Espresso->"Espresso Portafilter Basket", Kalita->"Kalita Wave 185 White", Moka Pot->blank, Chemex with a chosen filter keeps it. npm run check green (2.248.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.248.0)
## Sprint 261 (v2.249.0) - REDESIGN Move 4: recipe-aware auto-distribute (preserves pour shape)
- [ ] Burden erased: CORRECTION (sev 5) - "Auto-Distribute" flattened ANY recipe to equal pours, destroying intent (a bloom-heavy / tapered / 4:6 shape became flat).
- [ ] Fix (autoDistributePours): if the plain pours already carry weights (a shape), rescale them PROPORTIONALLY to the water instead of even-splitting; fall back to even only when blank. A deliberate bloom is kept (only defaulted when blank, to max 2x dose / 10% water). The last pour absorbs the rounding remainder so the schedule still sums EXACTLY to total water. Toast distinguishes "rescaled - shape kept" from "auto-distributed".
- [ ] Verified via preview: front-loaded [bloom60, 40,20,20] @250g -> bloom kept 60, plain rescaled [95,48,47] (first still biggest), sum 250; blank @250g/dose15 -> bloom default 30 + even [73,73,74], sum 250. npm run check green (2.249.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.249.0)
## Sprint 262 (v2.250.0) - REDESIGN Move 5: ambient dial-in nudge (AI stops waiting to be asked)
- [ ] Burden erased: DECISION (sev 5) - dial-in/diagnosis was a summon-a-panel button even though the signals (rating, EY-vs-sweetSpot, ratio-vs-cluster) are all already computed. Now the app proactively surfaces ONE quiet contextual line on the Brew tab when a cup landed off, with one tap into the existing dial-in.
- [ ] Fix (index.html): renderBrewNudge() in #todayHero (#todayNudge), called from renderToday. Priority: EY under/over the user's sweetSpot -> "Extraction 15.8% sits under your best cups (20.7-21.0%) - try finer/hotter"; else rating <=3 -> "This cup scored 2-star - the dial-in can pinpoint the one thing to change"; else ratio off the proven cluster -> "Your best V60s land around 1:Y - this one is 1:X". A "Dial in ->" button reuses openDialinFromNudge(). Read-only (never changes params); self-gates to silence with no signal (sweetSpot/ratioCluster need history, so new users never see a hollow nudge).
- [ ] Verified via preview: rating 2 -> rating nudge + button; rating 5 / no EY -> hidden; injected sweet spot 20.7-21.0% + cur EY 15.8 (rating 3 so it is excluded from the pool) -> EY-under nudge + button. npm run check green (2.250.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.250.0)
## Sprint 263 (v2.251.0) - REDESIGN Move 6: quick recipes carry their pour cadence
- [ ] Burden erased: INTERPRETATION - the per-method quick-recipe rails set dose/ratio/temp/grind only; the cadence that DEFINES a named method (4:6 etc.) lived in a tooltip technique string, so the guided timer + the new pour-plan card could not coach it.
- [ ] Fix (index.html): applyQuickRecipe now loads r.pours via setPourData when present, syncs totalWater to the pour sum, and sets totalBrewTime from r.time. Added accurate, original structured pours to the 3 marquee V60 recipes - Hoffmann (bloom 50 + to 100/150/200/250), Kasuya 4:6 (5x60 at ~45s, no separate bloom), April (bloom + 6x50 aggressive). Feeds Move 2 (pour-plan card) + Move 4 (recipe-aware distribute) + Brew-Along.
- [ ] Verified via preview: Hoffmann -> 5 pours sum 250 (Bloom 50 -> 100/150/200/250, done ~3:00); Kasuya -> 5x60 sum 300 (done ~3:30); April -> 6 pours sum 300 (done ~3:20). npm run check green (2.251.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.251.0)
## Sprint 264 (v2.252.0) - REDESIGN Move 7: grind setting travels across grinders
- [ ] Burden erased: CORRECTION/COORDINATION (sev 4) - switching grinders left a now-meaningless click number (25 on a Comandante 5-36 survived a switch to a J-Max 0-300, painting the caret far out of its pour-over band 150-210), forcing a manual re-lookup + retype.
- [ ] Fix (index.html): onGrinderChange(fromUser) now, on a real switch to a DIFFERENT known grinder, rescales grindSize by relative range position (pos in old range -> same pos in new range), so a pour-over grind stays a pour-over grind. _prevGrinder tracks the prior selection. Gated to fromUser=true (only the grinder select onchange passes it); the programmatic load/init calls (loadRecipeData, template apply) pass nothing, so a SAVED recipe grind is NEVER mutated on load. Transparent toast on change; no-op when the mapped value is unchanged.
- [ ] Verified via preview: Comandante 25 -> J-Max 194 (both pour-over); J-Max 180 -> Comandante 24 (pour-over band 20-30); same range -> unchanged (no toast); fromUser=false (load) -> unchanged (25 stays). npm run check green (2.252.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.252.0)
## Sprint 265 (v2.253.0) - REDESIGN Move 8: one-tap smart starting point (the app sets you up for the method)
- [ ] Burden erased: MEMORY/TYPING (sev 5, the marquee "understands before you explain") - a new recipe started from generic pour-over defaults regardless of method, so an espresso recipe began at dose 15 / 1:16 and you retyped dose/yield/shot/grind from memory.
- [ ] Fix (index.html): selectMethod now seeds the dials from the method canonical recipe (QUICK_RECIPES[method][0]: dose/ratio/temp/grind + espYield/espShot) via seedDialsForMethod() - QUIET, no toast. Gated to a blank new recipe: currentRecipeId===null && _blankNewRecipe. _blankNewRecipe is set in loadRecipeData (Object.keys(r).length===0) so it is TRUE only for a fresh newRecipe/first-run ({} data) and FALSE for a loaded recipe or a restored draft (populated data). Pick Espresso on a new recipe -> 18g/1:2/36g/27s; switch methods on a new recipe -> reshapes; restored draft or saved recipe being edited -> NEVER reshaped.
- [ ] Verified via preview: new recipe + Espresso -> 18/2/36/27; + V60 -> 15/16.7; draft (dose 22, _blankNewRecipe false) + pick Chemex -> 22 unchanged; saved recipe (id set, dose 19) + pick V60 -> 19 unchanged. npm run check green (2.253.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.253.0)
## Sprint 266 (v2.254.0) - REDESIGN Move 9: Kalita + Chemex carry their pour cadence
- [ ] Continues Move 6 (INTERPRETATION) - the major pour-over set now coaches its cadence: V60 (Move 6) + Chemex + Kalita.
- [ ] Fix (index.html): added accurate original structured pours to the marquee Chemex (Ultimate Chemex / Hoffmann: bloom 90 -> 300g -> 500g, done ~4:10) and Kalita Wave (Kalita 185: bloom 45 -> 150/250/320 pulse pours, last rinses the walls, done ~3:15) quick recipes. applyQuickRecipe (Move 6) loads them into the pour-plan card + Brew-Along.
- [ ] Verified via preview: Chemex -> 3 pours sum 500 (water 500); Kalita -> 4 pours sum 320 (water 320); plans render correctly. npm run check green (2.254.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.254.0)
## Sprint 267 (v2.255.0) - REDESIGN Move 10: diverse curated starter library (req #7)
- [ ] Burden erased: ONBOARDING/DISCOVERY - a new install seeded only 2 near-identical V60 recipes (hoffmann + kasuya), so the library felt empty + single-method and new users had nothing across methods to brew or learn from.
- [ ] Fix (index.html): added 2 new TEMPLATES - chemex (Ultimate Chemex: 30g/1:16.7/500g, bloom 90 -> 300 -> 500, Chemex filter) and espresso (Modern Espresso: 18g/1:2, 36g out / 28s, Espresso basket, no pours). Expanded the first-run seed from ['hoffmann','kasuya'] to ['hoffmann','kasuya','chemex','aeropress','frenchpress','espresso'] - 6 recipes across 5 methods (V60 x2, Chemex, AeroPress, French Press, Espresso). Only seeds genuinely-new installs (brewcraft_seeded gate); existing users unaffected.
- [ ] Verified via preview: all 6 templates well-formed; loadTemplate('chemex') -> Chemex/30g/500g/3 pours/Chemex filter; loadTemplate('espresso') -> Espresso/18g/36g yield/28s/Espresso basket. npm run check green (2.255.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.255.0)
## Sprint 268 (v2.256.0) - REDESIGN Move 11: grind descriptor -> actual clicks
- [ ] Burden erased: INTERPRETATION/TYPING (#9) - loading a recipe set the grind DESCRIPTOR (medium-fine) but left the grindSize click field blank even when a grinder was selected, so the user read "medium-fine" and had to know + type their grinder's number.
- [ ] Fix (index.html): new clicksForGrind(level) maps the 1-7 coarseness onto the selected grinder's own per-method band anchors (1->espresso fine end, 2->espresso mid, 3/4/5->pour-over low/mid/high, 6/7->french-press low/high). Wired into applyQuickRecipe + seedDialsForMethod: setting the descriptor now also fills grindSize with the real click number. Returns null for unknown/custom grinders (leaves the field alone). Also fixed the new Espresso template grind level 1->2 (Fine, not Extra-Fine/Turkish). Load path (loadRecipeData) untouched - saved recipes keep their stored grindSize.
- [ ] Verified via preview (Comandante C40): levels 1-7 -> 5/9/20/25/30/30/36; Hoffmann (medium-fine) -> 20 clicks; April (medium-coarse) -> 30 clicks (nuance preserved); Espresso -> level 2, 9 clicks. npm run check green (2.256.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.256.0)
## Sprint 269 (v2.257.0) - REDESIGN Move 12: grinder carries forward to new recipes (coherence fix)
- [ ] Found by an end-to-end coherence check of the new-recipe journey: doNewRecipe -> loadRecipeData({}) cleared grinderName, so a fresh recipe had NO grinder. That re-forced grinder selection every time AND silently disabled Move 11 grind-click translation on new recipes (clicks stayed blank because clicksForGrind found no grinder).
- [ ] Fix (index.html): doNewRecipe now carries the grinder forward - after loadRecipeData({}) it defaults grinderName to the most recently updated recipe's grinder (the user's hardware, not recipe-specific), then onGrinderChange() (no rescale - fromUser falsy). clearDirty() after keeps the new recipe clean. First-run (no recipes) -> stays blank; drafts keep their own grinder (loadRecipeData(draft) path untouched).
- [ ] Verified via preview: synthetic recent recipe with Comandante C40 -> doNewRecipe carries 'Comandante C40'; then pick V60 -> seeds dials AND fills clicks 20 (medium-fine). npm run check green (2.257.0 synced, boots clean, exit 0).
- [ ] Update toast after deploy (cache v2.257.0)
## Sprint 270 (v2.258.0) - REDESIGN Move 13: pour plan shows only for pour-over methods (coherence fix)
- [ ] Found by a per-method coherence check: the read-only pour-plan card (Move 2) + the Brew-Along derive only excluded Espresso + long-steep, so Moka Pot / Turkish / Flair (pressure/stovetop) and French Press / Clever / Cupping (immersion) all showed a bogus "Bloom 36g -> 162g -> 288g" schedule for methods that are not poured to a timed plan.
...
## Sprint 272 (v2.260.0) - REDESIGN Move 15: filter follows the method reshape on a blank new recipe (coherence fix)
- [ ] Found by a filter coherence check: switching methods on a blank new recipe reshaped the dials (Move 8) + grind clicks (Move 11) but NOT the filter (Move 3 maybeDefaultFilter only fills blank), so V60 -> Espresso left a V60 paper filter on the espresso recipe (shown on the Brew setup + saved).
...
## Sprint 274 (v2.262.0) - REDESIGN Move 17: Chemex seeds the right grind (coffee-correctness fix)
- [ ] Found by a per-method seed sanity check: every method smart-seed produced sensible dials EXCEPT Chemex, which seeded 'Coarse' (level 6). Cause: QUICK_RECIPES Chemex grind string 'medium (coarser than V60)' mis-parsed - grindDescriptorToLevel matches 'coarse' inside 'coarser'. Chemex should be medium-coarse; the wrong level also mis-targeted the grinder band for the Move 11 click translation (frenchPress band instead of pourOver).
...
## Sprint 276 (v2.264.0) - MERGE: unified Brew + Recipe into one screen (the major fix the owner flagged)
- [ ] Owner feedback: I shipped small connective tweaks but skated past the biggest structural ask - unifying the Brew and Recipe tabs (plus pour-editor redesign / pattern distribution / Orea / library breadth, queued next). This ships the unification.
- [ ] Stage 1 (structure): removed the Recipe nav button (5 tabs -> 4: Brew/Taste/Journal/Insights). switchTab now co-activates BOTH #tab-recipe + #tab-brew under the single Brew destination; 'recipe' and 'brew' are aliases of the merged screen, so all ~21 cross-tab links (Edit, library open, quick-brew, dial-in, bb2Tap, etc.) still land here. #tab-recipe active by default.
- [ ] Stage 2 (trim duplication): hid the read-only mirror in the brew header (#todaySubtitle/Name/Meta/Setup/PourPlan + the duplicate top #brewBar); removed the now-pointless "Edit recipe" button; added a "Brew" divider marking set-up -> brew; relocated "Switch recipe" out of the intelligence card; renderToday collapses #todayHero when it carries no brew-time intelligence (no empty box on bare recipes).
- [ ] Verified via preview DOM (screenshot tool hung all session): 4 tab buttons; merged screen co-activates recipe+brew with the full editor (method/dose/grind) + brew controls (stat/timer/pours/Brew-Along); other tabs switch cleanly; bare template -> intelligence card collapsed; freshness present -> card shown. npm run check green (2.264.0 synced, boots clean). Rollback tag: checkpoint-pre-merge-v2.263.0.
- [ ] Update toast after deploy (cache v2.264.0)
## Sprint 277 (v2.265.0) - #4: pour-schedule editor redesigned into a visual touch timeline
- [ ] Owner req #4: "redesign the pour schedule around human effort - rethink the interaction, not merely shorten; understand + edit at a glance; compact/visual/touch." Old editor = 4 stacked input fields per pour (weight/start/end/technique).
- [ ] Fix (index.html): rebuilt addPourRow into a compact card - a [-] [weight] [+] touch stepper (nudgePourWeight, 5g steps, haptic), the cumulative target (-> Xg), a small "at [time]" chip, an optional technique note; End preserved as a hidden input (lap-fill + round-trip). Added a proportional STRUCTURE BAR (#pourBar) above the cards, rendered by updatePourTotals: each pour a segment sized by weight, bloom highlighted - the build reads at a glance. CSS uses existing tokens (--accent/--accent-bright/--bg-deep). Data contract UNTOUCHED (.pour-weight/.pour-start/.pour-end/.pour-technique + dataset.isBloom + .pour-card-cum/.pour-card-title), so getPourData/setPourData/autoDistributePours/lapTimer/updatePourTotals keep working.
- [ ] Verified via preview DOM: setPourData([bloom45,105,100]) -> 3 cards with steppers + cumulative "-> 45g total" + bar with 3 proportional segments; getPourData round-trips weight/start/technique/isBloom; End preserved hidden; stepper + (105->110); auto-distribute intact. npm run check green (2.265.0). NOTE: screenshot tool hung all session - verified structurally; visual review pending.
- [ ] Update toast after deploy (cache v2.265.0)
## Sprint 278 (v2.266.0) - #5: pattern-aware auto-distribute (named pour shapes)
- [ ] Owner req #5: "auto-distribute must become recipe-aware... offer pattern-aware distribution: equal, bloom-heavy, Tetsu, Rao, etc. Do not flatten intentional recipes." Previously only shape-preserve/even.
...
## Sprint 280 (v2.268.0) - #6: AI recipe adaptation ("Adapt to my bean")
- [ ] Owner picked #6 direction 3: adapt a recipe to the bean/grinder with one-tap apply.
- [ ] Fix (index.html): "Adapt to my bean" button in the params AI row + adaptToBean() / ruleAdapt(). With an API key: aiComplete adapts the CURRENT recipe (dose/ratio/temp/grind +espresso yield/shot) to the bean origin/process/roast/freshness, strict JSON. No key: ruleAdapt computes a roast-level delta (light -> +2C & a step finer; dark -> -3C & a step coarser; medium unchanged) + a freshness note. Renders into the shared #suggestPanel with one-tap "Apply to dials" (reuses _lastSuggestion + applySuggestion).
- [ ] Verified via preview (rule path, no key; V60 15g/1:16.7/94C/med-fine): Light -> 96C + Fine; Dark -> 91C + Medium; Medium -> unchanged; panel "Adapted for your bean" + Apply; applying Dark sets 91C/level 4. npm run check green (2.268.0).
- [ ] Update toast after deploy (cache v2.268.0)
## Sprint 281 (v2.269.0) - #6 deeper: next-best-action guide
- [ ] Owner said "go deeper" on the AI layer. Added the persistent, quiet next-best-action prompt from the brief ("quiet next-best-action guidance").
- [ ] Fix (index.html): #nextAction bar at the top of the merged screen + renderNextAction() (called from renderToday). One contextual line, always one step ahead, state + method aware: blank recipe -> "pick a method" (scroll to sec-method); set + unrated + not brewed -> "Brew-Along" (or "To timer" for espresso/pressure, or steep guidance for long-steep); brewed but unrated -> "Rate it"; rated <=3 -> "Dial in" (openDialinFromNudge); rated >=4 -> "Brew again" (brewAgain). Uses currentRating/timerElapsed/brewCompleteFired.
- [ ] Verified via preview: blank -> Choose method; ready V60 -> Brew-Along; 2-star -> Dial in; 5-star -> Brew again; brewed+unrated -> Rate it. npm run check green (2.269.0).
- [ ] Update toast after deploy (cache v2.269.0)
## Sprint 282 (v2.270.0) - kill the setup scroll (live-feedback R1)
- [ ] Owner feedback (hands-on test): recipe setup was "three pages" of scrolling. Owner picked "kill the setup scroll first".
- [ ] Fix (index.html): (1) split the gear block (grinder/filter/grind/gear-profiles) OUT of the over-stuffed params section into its own collapsed #sec-gear; (2) renamed "Brew Parameters" -> "Dose & Ratio" (subtitle "Coffee, water & temperature"); (3) applyInitialDisclosure now collapses Bean for EVERYONE on boot (was beginner-only), boot-only so it never re-collapses mid-session; (4) added CSS order #sec-gear:4 + water:5 so Gear slots after Bean (it defaulted to order:0 and floated above Method - regression caught by screenshot).
- [ ] Result: setup opens to just Method + Dose & Ratio; Bean / Gear / Water collapsed (one tap). #tab-recipe 3475 -> 2620px. Method->filter default still fires (Chemex -> Chemex Bonded White). Opening Bean clears inert. npm run check green (2.270.0).
- [ ] Remaining (owner taste calls, deferred): dials -> "simple bubbles"; quick-recipes rework; header declutter + gear-icon-looks-like-a-sun; method icons not distinct + "V60" naming/taxonomy.
- [ ] Update toast after deploy (cache v2.270.0)
## Sprint 283 (v2.271.0) - Dose & Ratio: ruler dials -> compact bubbles (live-feedback R1)
- [ ] Owner wanted dose/ratio/water/temp as clean "simple bubbles" (like the brew-along pills), not tall slider rows; and the quick-recipes rail slimmed. Designed via a 4-design panel + adversarial critique + synthesis (steppers-bubbles won on wiring-safety + lowest impl-risk).
- [ ] Fix (index.html): replaced the .field-grid (4 .ruler drag-dials) with a 2x2 .bubble-grid of .param-bubble cards. Each keeps its real input (same id + oninput recalc) as the big tappable number; Dose/Water get -/+ steppers (new bump() snaps to step grid, fires the field's recalc cb, scale-pulse + haptic), Ratio/Temp get inline preset chips (setRatioChip/setTempChip unchanged). The 4 .ruler nodes + #dialHint removed; scrubber engine self-neutralizes (renderScrubber/initScrubber/resetField/refreshTempUI all guard on cfg.ticks/.ruler); showDialHint/dismissDialHint retired to no-ops. #tempUnitLabel KEPT (refreshTempUI deref, no guard). Each bubble keeps class="field param-bubble" so toggleEspressoFields still hides Ratio+Water for Espresso.
- [ ] Quick recipes: collapsed toggle (#qrecToggle / toggleQuickRecipes) + slim one-line pill strip (.qrec-strip); renderQuickRecipes re-collapses on method change; applyQuickRecipe UNCHANGED.
